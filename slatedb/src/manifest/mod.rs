...
impl ManifestCore {
    ...
    /// segment (root tree, RFC-0024 compatibility encoding) followed by each
    /// named segment in `segments` order. Use this when you need to identify
    /// which segment a tree belongs to (e.g. when building a `CompactionSpec`
    /// with `for_segment`); use [`trees`] when only the LSM state matters.
