...
/// Per-tree compaction candidates: prefix, source lists, and the backpressure
/// snapshot used to gate that tree's picks. Built once per `propose()` call
/// and reused across round-robin passes so the backpressure analysis isn't
...
impl CompactionScheduler for SizeTieredCompactionScheduler {
    fn propose(&self, state: &CompactorStateView) -> Vec<CompactionSpec> {
        ...
        let active_compactions = state
            ...
            .filter(|c| c.active())
            .collect::<Vec<_>>();
        let mut conflict_checker =
            ConflictChecker::new(active_compactions.iter().map(|j| j.spec()));
        // Sorted-run ids are globally unique across every tree, so fresh-id
        // allocation must skip past any in-flight destination as well as every
        // committed run in every tree.
        let mut next_fresh_sr_id = next_global_sr_id(db_state, &active_compactions);

        // Precompute per-tree (sources, backpressure) once. Trees are ordered
        // by L0 length descending — segments with more recent writes get first
        // dibs each round-robin pass. The empty-prefix segment (root tree,
        // RFC-0024 compatibility encoding) participates alongside named
        // segments. Stable sort preserves "root first" among ties so the
        // single-tree case behaves exactly as before.
        let mut trees: Vec<TreeCandidates> = db_state
            .trees_with_prefix()
            .map(|(prefix, tree)| {
        ...
        loop {
            ...
            for tree in &trees {
                ...
                let checker = CompactionChecker::new(conflict_checker.clone(), tree.bp.clone());
                if let Some(compaction) = self.pick_next_compaction(
                    &tree.prefix,
                    &tree.l0,
                    &tree.srs,
                    &checker,
                    ...
                    conflict_checker.add_compaction(&compaction);
...
impl SizeTieredCompactionScheduler {
    ...
    fn pick_next_compaction(
        ...
        if let Some(mut l0_candidates) = self.clamp_min(l0_candidates) {
            ...
            // Fresh L0 → SR destinations draw from a globally-unique counter
            // because SR ids are shared across every tree (RFC-0024).
            ...
            if checker.check_compaction(&l0_candidates, dst, next_sr) {
                *next_fresh_sr_id = next_fresh_sr_id.saturating_add(1);
                return Some(self.create_compaction(prefix, l0_candidates, dst));
            }
        }
        ...
        for i in 0..srs.len() {
            ...
            if let Some(mut compactable_run) = compactable_run {
                ...
                let dst = compactable_run
                    ...
                    .source
                    .unwrap_sorted_run();
                return Some(self.create_compaction(prefix, compactable_run, dst));
            }
        }
        None
    }
...
mod tests {
    ...
    fn test_should_distribute_picks_across_trees_round_robin() {
        ...
        // absorb four slots on its own under a depth-first walk.
        let root_l0: Vec<SsTableView> = (0..4).map(|_| create_sst_view(1)).collect();
        ...
        // Segment has only L0 work — depth-first would starve it.
        let seg_l0: Vec<SsTableView> = (0..4).map(|_| create_sst_view(1)).collect();
        core.segments = vec![segment_with(
            b"seg/",
            seg_l0.iter().cloned().collect(),
